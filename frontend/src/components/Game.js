...
const Game = () => {
  ...
  // Emotion-based background states
  ...
  // Smooth background animation loop
  useEffect(() => {
    let animationFrame;
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/child/game/game-1`)
      .then((response) => {
        setShuffledQuestions(response.data);
        setIsLoading(false);
      })
      .catch((error) => {
        console.error("Error fetching questions:", error);
  ...
  // Analyze emotion with compression
  const analyzeEmotion = async () => {
    if (!videoRef.current) return;

    // Create small offscreen canvas for compression
    ...
    const compressedDataUrl = offCanvas.toDataURL("image/jpeg", 0.5);
  ...
  const startCapture = () => {
    ...
    if (webcamGranted) {
      // Capture + analyze every 5 seconds (compressed)
      captureIntervalRef.current = setInterval(() => {
        captureImage(sessionId, sessionName, gameName);
        captureScreenshot(sessionId, sessionName, gameName);
        analyzeEmotion();
  ...
  const selectAnswer = (index, correct) => {
    ...
    setTimeout(() => {
      document.body.classList.remove("correct", "wrong");
      if (currentQuestionIndex + 1 < shuffledQuestions.length) setNextQuestion();
      else endGame();
    }, 1000);
  };
  ...
  const triggerConfetti = () => {
    ...
    const confettiInterval = setInterval(() => {
      createConfetti();
      particleCount -= 10;
      if (particleCount <= 0) clearInterval(confettiInterval);
    }, 30);
    setTimeout(() => clearInterval(confettiInterval), 10000);
  };
      ...
      {!webcamGranted ? (
        ...
        <div className="end-screen">
          ...
          <button
            className="btn"
            onClick={() =>
              navigate("/select-game", { state: { username, gameName } })
            }
          >
            Back to games
          </button>
