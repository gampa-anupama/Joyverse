...
//   const totalQuestions = allLevels.reduce((acc, level) => acc + level.length, 0);
...
//   // Reset background color on end screen
//   useEffect(() => {
//     if (showEndScreen) document.body.style.backgroundColor = "white";
//   }, [showEndScreen]);
...
//   const currentProgressIndex = allLevels
//     .slice(0, levelNumber - 1)
//     .reduce((acc, level) => acc + level.length, 0) + questionIndex;

//   const progressPercentage = Math.round((currentProgressIndex / totalQuestions) * 100);
...
//               role="progressbar"
//               style={{ width: `${progressPercentage}%`, backgroundColor: "#5a189a", transition: "width 0.5s ease" }}
...
const Game = () => {
  ...
  // Levels with words (images dynamically loaded from assets)
  const allLevels = [
    [
      { word: "cat", options: ["c", "a", "t"] },
      { word: "dog", options: ["d", "o", "g"] },
      { word: "sun", options: ["s", "u", "n"] },
    ],
    [
      { word: "fish", options: ["f", "i", "s", "h"] },
      { word: "milk", options: ["m", "i", "l", "k"] },
      { word: "cake", options: ["c", "a", "k", "e"] },
    ],
    [
      { word: "apple", options: ["a", "p", "p", "l", "e"] },
      { word: "tree", options: ["t", "r", "e", "e"] },
      { word: "book", options: ["b", "o", "o", "k"] },
    ],
  ];
  ...
  const handleDrop = (letter) => {
    ...
    if (letter === correctLetter) {
      setIsCorrect(true);
      speak("Correct!");
      setCurrentQuestion((prev) => ({ ...prev, word: prev.originalWord }));
      setCorrectStreak((prev) => prev + 1);
      setWrongStreak(0);
      setTimeout(() => nextQuestion(), 2000);
    } else {
      ...
      if (wrongStreak + 1 >= 2 && levelNumber > 1) {
        ...
        setCurrentQuestion(blankLetter(allLevels[prevLevel - 1][0]));
        setWrongStreak(0);
        setLevelMessage("Dropping a level to practice. You got this!");
        setTimeout(() => setLevelMessage(""), 3000);
      }
    }
  };
  ...
  const nextQuestion = () => {
    ...
    if (questionIndex < level.length - 1) {
      setQuestionIndex(questionIndex + 1);
      setCurrentQuestion(blankLetter(level[questionIndex + 1]));
      setIsCorrect(null);
    } else {
      if (levelNumber < allLevels.length) {
        const nextLevel = allLevels[levelNumber];
  ...
  return (
    <div className="game-container">
      <h3 className="mb-4 display-4" style={{ fontSize: "20px", fontFamily: "Comic Sans MS, sans-serif" }}>
        Level {levelNumber} - Fill in the Blank!
      </h3>
      ...
      {gameStarted && currentQuestion && (
        <>
          ...
          <WordWithImage
            ...
            missingIndex={currentQuestion.missingIndex}
            isCorrect={isCorrect}
            handleDrop={handleDrop}
          />
...
// Word + Image Component
const WordWithImage = ({ word, originalWord, missingIndex, isCorrect, handleDrop }) => {
  const onDrop = (e) => {
    e.preventDefault();
    const droppedLetter = e.dataTransfer.getData("letter");
    handleDrop(droppedLetter);
  ...
  // Dynamically load local JPEG image
  let imageSrc;
  try {
    imageSrc = require(`../assets/${originalWord.toLowerCase()}.jpeg`);
  } catch (err) {
  ...
  return (
    <div
      className="word-container"
      style={{ backgroundColor: getBackgroundColor(), borderRadius: "15px", transition: "background-color 0.3s ease", position: "relative", padding: "20px", textAlign: "center" }}
      onDrop={onDrop}
      onDragOver={(e) => e.preventDefault()}
    >
      <img
